      ...
      <div className="app" style={{ '--accent': co.color }}>
        ...
        <div className="sub-topbar">
          ...
          <div style={{ marginLeft: 'auto' }}>
            <button className="user-chip" onClick={logout}><div className={isAdmin ? 'dot-admin' : sess.role === 'ue' ? 'dot-ue' : 'dot-emp'} />{sess.name.split(' ')[0]}<span style={{ color: '#888', fontSize: 10 }}>× byt</span></button> className={`nav-btn${coTab === t.k ? ' active' : ''}`} onClick={() => setCoTab(t.k)}>{t.i}{t.l}</button>)}</nav>
  ...
  return (
    <div className="app" style={{ '--accent': '#1a1a1a' }}>
      ...
      <nav className="nav">
        ...
        {isAdmin && <button className={`nav-btn${mainTab === 'sms' ? ' active' : ''}`} onClick={() => setMainTab('sms')}><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" width="16" height="16"><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/></svg>SMS</button>}
      </nav>
      <div className="content">
