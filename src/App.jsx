...
export default function App() {
  ...
  if (activeCo && co) {
    ...
    if (isTransport) {
      ...
      tabs = [
        ...
        { k: 'svets', l: 'Svetsprotokoll', i: <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" width="16" height="16"><path d="M12 2L2 7l10 5 10-5-10-5z"/><path d="M2 17l10 5 10-5"/><path d="M2 12l10 5 10-5"/></svg> },
        ...(isAdmin ? [{ k: 'rapport', l: 'Rapport', i: Icon.rap }] : [])
      ]
    } else {
      tabs = [
  ...
  return (
    <div className="app" style={{ '--accent': '#1a1a1a' }}>
      ...
      {showSettings && <SettingsSheet employees={employees} addEmployee={addEmployee} updateEmployee={updateEmployee} deleteEmployee={deleteEmployee} kunder={kunder} addKund={addKund} deleteKund={deleteKund} onClose={() => setShowSettings(false)} />}
...
// ─── Settings Sheet ───────────────────────────────────────────
function SettingsSheet({ employees, addEmployee, updateEmployee, deleteEmployee, kunder, addKund, deleteKund, onClose }) {
  ...
  const startEdit = (emp) => {
    setEditId(emp.id)
    setForm({ name: emp.name, role: emp.role, pin: emp.pin, phone: emp.phone || '' })
    setView('edit')
  }
    ...
    <div className="overlay" onClick={onClose}>
      <div className="sheet" onClick={e => e.stopPropagation()}>
        ...
        {section === 'personal' && (
          <div>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
              <div style={{ fontSize: 15, fontWeight: 700 }}>{view === 'add' ? 'Lägg till anställd' : view === 'edit' ? 'Redigera anställd' : 'Personal'}</div>
            ...
            {view === 'add' && empForm(saveEmp, () => { setView('list'); setForm({ name: '', role: 'employee', pin: '', phone: '' }); setPinErr('') }, false)}
            {view === 'edit' && empForm(saveEdit, () => { setView('list'); setEditId(null); setForm({ name: '', role: 'employee', pin: '', phone: '' }); setPinErr('') }, true)}
          </div>
        )}
        ...
        {section === 'kunder' && (
          <div>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
              <div style={{ fontSize: 15, fontWeight: 700 }}>{view === 'add' ? 'Lägg till kund' : 'Kundlista'}</div>
                ...
                {kunder.map(k => (
                  <div key={k.id} className="emp-row">
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      <div style={{ fontSize: 11, color: '#888', marginTop: 2 }}>{k.contact}{k.phone ? ' · 📞 ' + k.phone : ''}</div>
                    </div>
            ...
            {view === 'add' && (
              <div>
                ...
                <div style={{ display: 'flex', gap: 8 }}>
                  <button className="btn-primary" style={{ flex: 1, marginTop: 0 }} onClick={saveKund}>Spara</button>
                  <button style={{ marginTop: 0, padding: '10px 16px', background: 'transparent', border: '0.5px solid rgba(0,0,0,.2)', borderRadius: 8, fontSize: 14, cursor: 'pointer', fontFamily: 'inherit' }} onClick={() => { setView('list'); setKundForm({ name: '', contact: '', phone: '', email: '' }) }}>Avbryt</button>
                </div>
              </div>
            )}
          </div>
...
function VeckoplaningTab({ sess, isAdmin, veckoplanning, employees, projects, addVeckopost, updateVeckopost, deleteVeckopost }) {
  const [showForm, setShowForm] = useState(false)
  const [form, setForm] = useState({ datum: today(), employee_id: '', employee_name: '', project_id: '', project_name: '', notering: '', company: '' })
  ...
  const submit = async () => {
    if (!form.datum || !form.employee_name) return
    await addVeckopost({ ...form })
    setForm({ datum: today(), employee_id: '', employee_name: '', project_id: '', project_name: '', notering: '', company: '' })
    setShowForm(false)
  }

  // Filter to show only current user's posts if not admin
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
        ...
        {isAdmin && !showForm && <button style={{ background: '#1a1a1a', border: 'none', color: '#fff', borderRadius: 8, padding: '7px 12px', fontSize: 12, fontWeight: 600, cursor: 'pointer' }} onClick={() => setShowForm(true)}>+ Lägg till</button>}
      </div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: '#fff', borderRadius: 10, padding: '8px 12px', marginBottom: 12, border: '0.5px solid rgba(0,0,0,.1)' }}>
        <button onClick={() => setWeekOffset(w => w - 1)} style={{ background: 'none', border: 'none', fontSize: 18, cursor: 'pointer', color: '#888' }}>‹</button>
        <div style={{ fontSize: 13, fontWeight: 600 }}>
          {weekOffset === 0 ? 'Denna vecka' : weekOffset === 1 ? 'Nästa vecka' : weekOffset === -1 ? 'Förra veckan' : `Vecka ${new Date(weekDates[0]).toLocaleDateString('sv-SE', { day: 'numeric', month: 'short' })}`}
        </div>
        <button onClick={() => setWeekOffset(w => w + 1)} style={{ background: 'none', border: 'none', fontSize: 18, cursor: 'pointer', color: '#888' }}>›</button>
      </div>
      ...
      {isAdmin && showForm && (
        <div className="card" style={{ marginBottom: 12 }}>
          ...
          <div className="field-group"><div className="field-label">Anställd *</div>
            <select className="field" value={form.employee_id} onChange={e => {
          ...
          <div className="field-group"><div className="field-label">Notering</div><input className="field" placeholder="T.ex. Dag 1 av 3, hämta verktyg 07:00" value={form.notering} onChange={e => setForm({ ...form, notering: e.target.value })} /></div>
          <div style={{ display: 'flex', gap: 8 }}>
            <button className="btn-primary" style={{ flex: 1, marginTop: 0, background: '#1a1a1a' }} onClick={submit}>Spara</button>
            <button className="btn-secondary" style={{ marginTop: 0 }} onClick={() => setShowForm(false)}>Avbryt</button>
          </div>
        </div>
      )}
      ...
      {weekDates.map((d, di) => {
        ...
        return (
          <div key={d} style={{ marginBottom: 8 }}>
            <div style={{ fontSize: 12, fontWeight: 600, color: isToday ? '#E05D1A' : isPast ? '#bbb' : '#666', marginBottom: 4, display: 'flex', alignItems: 'center', gap: 6 }}>
            ...
            {posts.length === 0 ? (
              ...
              posts.map(p => (
                <div key={p.id} style={{ background: '#fff', border: `0.5px solid ${isToday ? '#E05D1A44' : 'rgba(0,0,0,.08)'}`, borderRadius: 8, padding: '8px 12px', marginBottom: 4 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                    <div>
                      {isAdmin && <div style={{ fontSize: 11, color: '#888', marginBottom: 2 }}>👤 {p.employee_name}</div>}
                      <div style={{ fontSize: 14, fontWeight: 600 }}>{p.project_name || '—'}</div>
                      {p.company && <div style={{ fontSize: 11, color: '#888', marginTop: 2 }}>{p.company}</div>}
                      {p.notering && <div style={{ fontSize: 12, color: '#666', marginTop: 3 }}>{p.notering}</div>}
                    </div>
                    {isAdmin && <button className="btn-danger" style={{ padding: '3px 6px' }} onClick={() => deleteVeckopost(p.id)}>{Icon.del}</button>}
                  </div>
                </div>
              ))
            )}
