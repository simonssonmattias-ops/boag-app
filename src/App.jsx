  ...
  if (view === 'detail' && proj) {
    ...
    return (
      <div>
        ...
        {ptab === 'ekonomi' && isAdmin && <EkonomiPanel projId={proj.id} co={co} />}
      </div>
    )
  }
...
function EkonomiPanel({ projId, co }) {
  ...
  useEffect(() => {
    supabase.from('projektfaktura').select('*').eq('project_id', projId).order('datum', { ascending: false })
      .then(({ data }) => { setFakturor(data || []); setLoaded(true) })
  }, [projId])
  ...
  const del = async (id) => {
    await supabase.from('projektfaktura').delete().eq('id', id)
  ...
  const totalt = fakturor.reduce((s, f) => s + (f.belopp || 0), 0)
  const betalt = fakturor.filter(f => f.status === 'betald').reduce((s, f) => s + (f.belopp || 0), 0)
  const obetalt = totalt - betalt
  ...
  return (
    <div>
      <div className="stat-grid" style={{ marginBottom: 12 }}>
        <div className="stat-card"><div className="stat-label">Totalt fakturerat</div><div className="stat-val" style={{ color: co.color, fontSize: 15 }}>{totalt.toLocaleString('sv-SE')} kr</div></div>
        <div className="stat-card"><div className="stat-label">Betalt</div><div className="stat-val" style={{ fontSize: 15, color: '#1a7a3c' }}>{betalt.toLocaleString('sv-SE')} kr</div></div>
      </div>
      {obetalt > 0 && <div style={{ fontSize: 12, color: '#888', marginBottom: 10 }}>Obetalt: <strong style={{ color: '#1a1a1a' }}>{obetalt.toLocaleString('sv-SE')} kr</strong></div>}
