  ...
  if (activeCo && co) {
    ...
    if (isTransport) {
      ...
      tabs = [
        ...
        { k: 'arbetsdag', l: 'Arbetsdag', i: Icon.dag },
    ...
    return (
      <div className="app" style={{ '--accent': co.color }}>
        ...
        <div className="content">
          ...
          {/* Transport */}
          {coTab === 'korsedlar' && <KorsedlarTab co={co} korsedlar={cKorsedlar} kunder={kunder} employees={employees} sess={sess} isAdmin={isAdmin} addKorsedel={addKorsedel} deleteKorsedel={deleteKorsedel} />}
          {/* MB Mark */}
  ...
  return (
    <div className="app" style={{ '--accent': '#1a1a1a' }}>
      ...
      <div className="content">
        ...
        {mainTab === 'anbud' && isAdmin && <AnbudTab anbud={anbud} addAnbud={addAnbud} updateAnbud={updateAnbud} deleteAnbud={deleteAnbud} />}
      </div>
    </div>
  )
}
...
// ─── Anbud Tab ────────────────────────────────────────────────
const ANBUD_STATUS = {
  KALKYLERAS: { label: 'Kalkyleras', color: '#B84B12', bg: '#FEF0E6' },
  INLAMNAT:   { label: 'Inlämnat',   color: '#1a6ab5', bg: '#E8F0FE' },
  VUNNIT:     { label: 'Vunnit',     color: '#1a7a3c', bg: '#E6F7EE' },
  FORLORAT:   { label: 'Förlorat',   color: '#a32d2d', bg: '#FEECEC' },
}

function AnbudTab({ anbud, addAnbud, updateAnbud, deleteAnbud }) {
  const [view, setView] = useState('dashboard')
  const [filterStatus, setFilterStatus] = useState('ALL')
  const [search, setSearch] = useState('')
  const [editId, setEditId] = useState(null)
  const [form, setForm] = useState({ projektnamn: '', bestallare: '', ort: '', anbudssumma: '', anbudsdatum: '', status: 'KALKYLERAS', tilldelning: '', notering: '' })

  const fmtKr = (n) => { if (!n) return '–'; return Number(n).toLocaleString('sv-SE', { maximumFractionDigits: 0 }) + ' kr' }
  const fmtM = (n) => { if (!n) return '–'; return (Number(n)/1000000).toLocaleString('sv-SE', { minimumFractionDigits: 1, maximumFractionDigits: 1 }) + ' Mkr' }

  const stats = {
    total: anbud.length,
    vunnit: anbud.filter(a => a.status === 'VUNNIT').length,
    inlamnat: anbud.filter(a => a.status === 'INLAMNAT').length,
    kalkyleras: anbud.filter(a => a.status === 'KALKYLERAS').length,
    hitRate: anbud.length > 0 ? Math.round((anbud.filter(a => a.status === 'VUNNIT').length / anbud.length) * 100) : 0,
    totalAnbud: anbud.reduce((s, a) => s + (Number(a.anbudssumma) || 0), 0),
    vunnetVarde: anbud.filter(a => a.status === 'VUNNIT').reduce((s, a) => s + (Number(a.anbudssumma) || 0), 0),
  }

  const filtered = anbud.filter(a => {
    const matchStatus = filterStatus === 'ALL' || a.status === filterStatus
    const q = search.toLowerCase()
    return matchStatus && (!q || a.projektnamn?.toLowerCase().includes(q) || a.bestallare?.toLowerCase().includes(q) || a.ort?.toLowerCase().includes(q))
  }).sort((a, b) => new Date(b.anbudsdatum) - new Date(a.anbudsdatum))

  const submit = async () => {
    if (!form.projektnamn.trim() || !form.anbudssumma || !form.bestallare.trim()) return
    if (editId) {
      await updateAnbud(editId, { ...form })
    } else {
      await addAnbud({ ...form })
    }
    setForm({ projektnamn: '', bestallare: '', ort: '', anbudssumma: '', anbudsdatum: '', status: 'KALKYLERAS', tilldelning: '', notering: '' })
    setEditId(null); setView('list')
  }

  const startEdit = (a) => { setForm({ ...a }); setEditId(a.id); setView('form') }
  const startNew = () => { setForm({ projektnamn: '', bestallare: '', ort: '', anbudssumma: '', anbudsdatum: today(), status: 'KALKYLERAS', tilldelning: '', notering: '' }); setEditId(null); setView('form') }

  if (view === 'form') return (
    <div>
      <button className="btn-back" onClick={() => { setView('list'); setEditId(null) }}>{Icon.back} Tillbaka</button>
      <div style={{ fontSize: 16, fontWeight: 700, marginBottom: 14 }}>{editId ? 'Redigera anbud' : 'Nytt anbud'}</div>
      <div className="card">
        <div className="field-group"><div className="field-label">Projektnamn *</div><input className="field" placeholder="T.ex. Skogsgläntan, Mölnlycke" value={form.projektnamn} onChange={e => setForm(f => ({...f, projektnamn: e.target.value}))} autoFocus /></div>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
          <div className="field-group"><div className="field-label">Beställare *</div><input className="field" placeholder="T.ex. Förbo AB" value={form.bestallare} onChange={e => setForm(f => ({...f, bestallare: e.target.value}))} /></div>
          <div className="field-group"><div className="field-label">Ort</div><input className="field" placeholder="T.ex. Mölnlycke" value={form.ort} onChange={e => setForm(f => ({...f, ort: e.target.value}))} /></div>
          <div className="field-group"><div className="field-label">Anbudssumma (kr) *</div><input className="field" type="number" placeholder="T.ex. 1813895" value={form.anbudssumma} onChange={e => setForm(f => ({...f, anbudssumma: e.target.value}))} /></div>
          <div className="field-group"><div className="field-label">Anbudsdatum</div><input className="field" type="date" value={form.anbudsdatum} onChange={e => setForm(f => ({...f, anbudsdatum: e.target.value}))} /></div>
          <div className="field-group"><div className="field-label">Status</div>
            <select className="field" value={form.status} onChange={e => setForm(f => ({...f, status: e.target.value}))}>
              {Object.entries(ANBUD_STATUS).map(([k, s]) => <option key={k} value={k}>{s.label}</option>)}
            </select>
          </div>
          {form.status === 'VUNNIT' && <div className="field-group"><div className="field-label">Tilldelning (%)</div><input className="field" type="number" min="0" max="200" placeholder="T.ex. 92" value={form.tilldelning} onChange={e => setForm(f => ({...f, tilldelning: e.target.value}))} /></div>}
        </div>
        <div className="field-group"><div className="field-label">Notering</div><textarea className="field" rows={3} placeholder="Kommentar, varför vi vann/förlorade..." value={form.notering} onChange={e => setForm(f => ({...f, notering: e.target.value}))} /></div>
        {form.anbudssumma && <div style={{ background: '#F0EFE8', borderRadius: 8, padding: '10px 12px', marginBottom: 10, display: 'flex', justifyContent: 'space-between' }}><span style={{ fontSize: 12, color: '#888' }}>Anbudssumma</span><span style={{ fontSize: 18, fontWeight: 700 }}>{fmtKr(form.anbudssumma)}</span></div>}
        <div style={{ display: 'flex', gap: 8 }}>
          <button className="btn-primary" style={{ flex: 1, marginTop: 0, background: '#1a1a1a' }} onClick={submit}>{editId ? 'Spara ändringar' : 'Lägg till anbud'}</button>
          <button className="btn-secondary" style={{ marginTop: 0 }} onClick={() => { setView('list'); setEditId(null) }}>Avbryt</button>
        </div>
      </div>
    </div>
  )
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
        <div>
          <div style={{ fontSize: 15, fontWeight: 700 }}>Anbudssystem</div>
          <div style={{ fontSize: 12, color: '#888', marginTop: 1 }}>Kalkyleras, inlämnat, vunnet</div>
        </div>
        <div style={{ display: 'flex', gap: 6 }}>
          <button className="btn-secondary" style={{ fontSize: 12 }} onClick={() => setView(view === 'dashboard' ? 'list' : 'dashboard')}>{view === 'dashboard' ? '📋 Lista' : '📊 Dashboard'}</button>
          <button style={{ background: '#1a1a1a', border: 'none', color: '#fff', borderRadius: 8, padding: '7px 12px', fontSize: 12, fontWeight: 600, cursor: 'pointer' }} onClick={startNew}>+ Nytt anbud</button>
        </div>
      </div>
      ...
      {view === 'dashboard' && (
        <div>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginBottom: 12 }}>
            {[
              { l: 'Totalt anbud', v: stats.total, sub: fmtM(stats.totalAnbud) },
              { l: 'Hit-rate', v: stats.hitRate + '%', sub: stats.vunnit + ' vunna' },
              { l: 'Vunnet värde', v: fmtM(stats.vunnetVarde), sub: stats.vunnit + ' projekt' },
              { l: 'Under kalkyl', v: stats.kalkyleras, sub: stats.inlamnat + ' inlämnade' },
            ].map(({ l, v, sub }) => (
              <div key={l} className="stat-card"><div className="stat-label">{l}</div><div className="stat-val" style={{ fontSize: 18 }}>{v}</div><div style={{ fontSize: 11, color: '#888', marginTop: 2 }}>{sub}</div></div>
            ))}
          </div>
          <div className="sec">Senaste anbud</div>
          {anbud.slice(0, 5).map(a => {
            const st = ANBUD_STATUS[a.status]
            return (
              <div key={a.id} className="card" style={{ marginBottom: 8, cursor: 'pointer' }} onClick={() => startEdit(a)}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                  <div><div style={{ fontSize: 14, fontWeight: 700 }}>{a.projektnamn}</div><div style={{ fontSize: 12, color: '#888', marginTop: 2 }}>{a.bestallare}{a.ort ? ' · ' + a.ort : ''}</div></div>
                  <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 4 }}>
                    <span style={{ fontSize: 11, padding: '2px 8px', borderRadius: 20, fontWeight: 600, background: st.bg, color: st.color }}>{st.label}</span>
                    <span style={{ fontSize: 13, fontWeight: 700 }}>{fmtKr(a.anbudssumma)}</span>
                  </div>
                </div>
              </div>
            )
      ...
      {view === 'list' && (
        <div>
          <input className="field" placeholder="🔍 Sök projekt, beställare..." value={search} onChange={e => setSearch(e.target.value)} style={{ marginBottom: 10 }} />
          <div className="chip-group" style={{ marginBottom: 12 }}>
            <div className={`chip${filterStatus === 'ALL' ? ' selected' : ''}`} onClick={() => setFilterStatus('ALL')}>Alla ({anbud.length})</div>
            {Object.entries(ANBUD_STATUS).map(([k, s]) => (
              <div key={k} className={`chip${filterStatus === k ? ' selected' : ''}`} style={filterStatus === k ? { background: s.color, borderColor: s.color } : {}} onClick={() => setFilterStatus(k)}>
                {s.label} ({anbud.filter(a => a.status === k).length})
              </div>
            ))}
          </div>
          ...
          {filtered.map(a => {
            ...
            return (
              <div key={a.id} className="card" style={{ marginBottom: 8 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                  <div style={{ flex: 1 }}>
                    <div style={{ fontSize: 14, fontWeight: 700 }}>{a.projektnamn}</div>
                    <div style={{ fontSize: 12, color: '#888', marginTop: 2 }}>{a.bestallare}{a.ort ? ' · ' + a.ort : ''}{a.anbudsdatum ? ' · ' + fmt(a.anbudsdatum) : ''}</div>
                    {a.notering && <div style={{ fontSize: 12, color: '#666', marginTop: 4, fontStyle: 'italic' }}>{a.notering}</div>}
                  </div>
                  <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 5, marginLeft: 10 }}>
                    <span style={{ fontSize: 11, padding: '2px 8px', borderRadius: 20, fontWeight: 600, background: st.bg, color: st.color }}>{st.label}</span>
                    <span style={{ fontSize: 14, fontWeight: 700 }}>{fmtKr(a.anbudssumma)}</span>
                    {a.tilldelning && <span style={{ fontSize: 11, color: '#1a7a3c' }}>Tilldeln. {a.tilldelning}%</span>}
                    <div style={{ display: 'flex', gap: 5 }}>
                      <button style={{ padding: '3px 7px', background: 'transparent', border: '0.5px solid rgba(0,0,0,.2)', borderRadius: 7, cursor: 'pointer', fontSize: 12 }} onClick={() => startEdit(a)}>✏️</button>
                      <button className="btn-danger" style={{ padding: '3px 6px' }} onClick={() => deleteAnbud(a.id)}>{Icon.del}</button>
                    </div>
                  </div>
                </div>
              </div>
...
function AnbudTab() {
  ...
  const [form, setForm] = useState({ projektnamn: '', bestallare: '', ort: '', anbudssumma: '', anbudsdatum: today(), status: 'KALKYLERAS', tilldelning: '', notering: '' })
  const [loaded, setLoaded] = useState(false)
  ...
  const save = async () => {
    ...
    setForm({ projektnamn: '', bestallare: '', ort: '', anbudssumma: '', anbudsdatum: today(), status: 'KALKYLERAS', tilldelning: '', notering: '' })
    setEditId(null); setView('list')
  }
  ...
  const startEdit = (p) => { setForm({ ...p, anbudssumma: String(p.anbudssumma) }); setEditId(p.id); setView('form') }
  ...
  if (view === 'form') return (
    <div>
      <button className="btn-back" onClick={() => { setView('list'); setEditId(null); setForm({ projektnamn: '', bestallare: '', ort: '', anbudssumma: '', anbudsdatum: today(), status: 'KALKYLERAS', tilldelning: '', notering: '' }) }}>{Icon.back} Tillbaka</button>
      <div style={{ fontSize: 16, fontWeight: 700, marginBottom: 14 }}>{editId ? 'Redigera anbud' : 'Nytt anbud'}</div>
          ...
          <div key={p.id} className="card" style={{ marginBottom: 8 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
              <div style={{ flex: 1 }}>
                ...
                <div style={{ fontSize: 12, color: '#888', marginTop: 2 }}>{p.bestallare}{p.ort ? ' · ' + p.ort : ''}{p.anbudsdatum ? ' · ' + fmt(p.anbudsdatum) : ''}</div>
...
function KorsedlarTab({ co, korsedlar, kunder, employees, sess, isAdmin, addKorsedel, deleteKorsedel }) {
  const [view, setView] = useState('list')
  ...
  const submit = async () => {
    ...
    await addKorsedel({ ...form, company: co.id, antal_lass: parseFloat(form.antal_lass) || 0, timmar: parseFloat(form.timmar) || 0, pris: parseFloat(form.pris) || 0 })
  ...
  const totalH = korsedlar.reduce((s, k) => s + (k.timmar || 0), 0)
  const totalLass = korsedlar.reduce((s, k) => s + (k.antal_lass || 0), 0)

  if (view === 'add') return (
    <div>
      <button className="btn-back" onClick={() => setView('list')}>{Icon.back} Tillbaka</button>
      <div style={{ fontSize: 16, fontWeight: 700, marginBottom: 14 }}>Ny körsedel</div>
      ...
      <div className="field-group"><div className="field-label">Kund *</div>
        {kunder.length === 0
          ...
          : <div className="chip-group">{kunder.map(k => <div key={k.id} className={`chip${form.kund_id === k.id ? ' selected' : ''}`} onClick={() => setForm({ ...form, kund_id: k.id })}>{k.name}</div>)}</div>}
      </div>
      ...
      <button className="btn-primary" onClick={submit} disabled={!form.kund_id}>Spara körsedel</button>
    </div>
  )
  ...
  return (
    <div>
      <div className="stat-grid">
        <div className="stat-card"><div className="stat-label">Totalt timmar</div><div className="stat-val" style={{ color: co.color }}>{totalH}h</div></div>
        <div className="stat-card"><div className="stat-label">Totalt lass</div><div className="stat-val">{totalLass}</div></div>
      </div>
      {korsedlar.length === 0 && <div className="empty">Inga körsedlar ännu.</div>}
      {korsedlar.map(k => {
        const kund = getKund(k.kund_id)
        const canDel = isAdmin || k.forare === sess.name
        return (
          <div key={k.id} className="entry-card">
            <div className="card-row">
              <div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 4 }}>
                ...
                {canDel && <button className="btn-danger" style={{ padding: '3px 6px' }} onClick={() => deleteKorsedel(k.id)}>{Icon.del}</button>}
              </div>
            </div>
...
function TransportRapportTab({ co, korsedlar, kunder }) {
  ...
  korsedlar.forEach(k => {
    ...
    byForare[k.forare].timmar += k.timmar || 0
  ...
  const downloadPDF = async () => {
    ...
    try {
      ...
      Object.entries(byKund).forEach(([namn, data]) => {
        if (y > 260) { doc.addPage(); y = margin }
        doc.setFontSize(11); doc.setFont('helvetica', 'bold'); doc.setTextColor(45, 143, 78)
        doc.text(namn, margin, y); y += 5
        doc.setDrawColor(200, 200, 200); doc.line(margin, y, margin + cw, y); y += 4
