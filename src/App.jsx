    ...
    return (
      <div className="app" style={{ '--accent': co.color }}>
        ...
        <div className="content">
          {/* Mark & Bygg */}
          {coTab === 'projects' && <ProjektTab co={co} projects={cProjects} allProjects={projects} employees={employees} sess={sess} isAdmin={isAdmin} dagbok={cDagbok} tid={cTid} ata={cAta} files={files} addProject={addProject} updateProject={updateProject} deleteProject={deleteProject} addAta={addAta} updateAta={updateAta} deleteAta={deleteAta} addFile={addFile} deleteFile={deleteFile} setLb={setLb} addDagbok={addDagbok} addTid={addTid} deleteDagbok={deleteDagbok} deleteTid={deleteTid} />}
...
// ─── Projekt Tab ──────────────────────────────────────────────
function ProjektTab({ co, projects, allProjects, employees, sess, isAdmin, dagbok, tid, ata, files, addProject, updateProject, deleteProject, addAta, updateAta, deleteAta, addFile, deleteFile, setLb, addDagbok, addTid, deleteDagbok, deleteTid }) {
  const [view, setView] = useState('list')
  ...
  if (view === 'detail' && proj) {
    ...
    return (
      <div>
        ...
        {ptab === 'arbetsdag' && <ProjektArbetsdagPanel proj={proj} co={co} dagbok={pDag} tid={pTid} employees={employees} sess={sess} isAdmin={isAdmin} addDagbok={addDagbok} addTid={addTid} deleteDagbok={deleteDagbok} deleteTid={deleteTid} />}
        {ptab === 'info' && isAdmin && (
          <div>
        ...
        <div onClick={() => levFileRef.current?.click()} style={{ border: '1.5px dashed rgba(0,0,0,.2)', borderRadius: 10, padding: 18, textAlign: 'center', cursor: 'pointer', marginBottom: 14, background: '#FAFAF8' }}>
          {levScanning ? (
            ...
            <div>
              ...
              <div style={{ fontSize: 12, color: '#1a7a3c' }}>✓ Faktura inläst — kontrollera uppgifterna nedan</div>
            </div>
          ) : (
            <div>
...
function ProjektArbetsdagPanel({ proj, co, dagbok, tid, employees, sess, isAdmin, addDagbok, addTid, deleteDagbok, deleteTid }) {
  const [view, setView] = useState('list')
  ...
  const submit = async () => {
    if (form.text.trim()) {
      await addDagbok({ project_id: proj.id, company: co.id, date: form.date, employee: form.employee, text: form.text, photos: form.photos })
    }
    if (form.hours) {
      await addTid({ project_id: proj.id, company: co.id, date: form.date, employee: form.employee, hours: parseFloat(form.hours) || 0, description: form.description })
    }
    for (const ep of form.extraPersons) {
      if (ep.name && ep.hours) {
        await addTid({ project_id: proj.id, company: co.id, date: form.date, employee: ep.name, hours: parseFloat(ep.hours) || 0, description: form.description })
      }
    }
    setForm({ date: today(), employee: sess.name, text: '', photos: [], hours: '8.5', description: '', extraPersons: [] })
    setView('list')
  }
  ...
  if (view === 'add') return (
    <div>
      <button className="btn-back" onClick={() => setView('list')}>{Icon.back} Tillbaka</button>
      <div style={{ fontSize: 16, fontWeight: 700, marginBottom: 14 }}>Ny arbetsdag — {proj.name}</div>
      ...
      <div className="field-group">
        ...
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 8 }}>
          {form.photos.map((p, i) => <img key={i} src={p} style={{ width: 60, height: 60, objectFit: 'cover', borderRadius: 8 }} />)}
        </div>
        <div style={{ display: 'flex', gap: 8 }}>
      ...
      <button className="btn-primary" style={{ background: co.color }} onClick={submit}>Spara</button>
    </div>
  )
  ...
  return (
    <div>
      ...
      <button className="btn-primary" style={{ background: co.color, marginBottom: 12 }} onClick={() => setView('add')}>+ Ny arbetsdag</button>
      ...
      {combined.map(d => (
        <div key={d.id} className="entry-card" style={{ marginBottom: 8 }}>
          <div className="card-row">
            ...
            {(isAdmin || d.employee === sess.name) && <button className="btn-danger" style={{ padding: '3px 6px' }} onClick={() => deleteDagbok(d.id)}>{Icon.del}</button>}
          </div>
          {d.photos?.length > 0 && <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginTop: 6 }}>{d.photos.map((p, i) => <img key={i} src={p} style={{ width: 50, height: 50, objectFit: 'cover', borderRadius: 6 }} />)}</div>}
        </div>
      ))}
    </div>
  )
  ...
  return (
    <div>
      ...
      {filtered.map(k => {
        ...
        return (
          <div key={k.id} className="entry-card" style={k.fakturerad ? { opacity: 0.6 } : {}}>
            <div className="card-row">
              <div>
            ...
            <div style={{ display: 'flex', gap: 8, marginTop: 6, flexWrap: 'wrap', alignItems: 'center' }}>
              ...
              {isAdmin && (
                <button
                  onClick={() => updateKorsedel(k.id, { fakturerad: !k.fakturerad })}
                  style={{ marginLeft: 'auto', fontSize: 11, fontWeight: 600, padding: '4px 10px', borderRadius: 20, border: 'none', cursor: 'pointer', fontFamily: 'inherit', background: k.fakturerad ? '#E6F7EE' : '#1a1a1a', color: k.fakturerad ? '#1a7a3c' : '#fff' }}
                >
