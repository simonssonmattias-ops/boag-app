  ...
  const downloadLevPDF = async () => {
    ...
    try {
      ...
      stats.forEach((s, i) => {
        ...
        doc.setFontSize(8); doc.setTextColor(120, 120, 120); doc.setFont('helvetica', 'normal')
        doc.text(s.l, x + sw / 2, y + 5, { align: 'center' })
        doc.setFontSize(12); doc.setTextColor(0, 0, 0); doc.setFont('helvetica', 'bold')
        doc.text(s.v, x + sw / 2, y + 12, { align: 'center' })
      })
      y += 24
      ...
      doc.text('Fakturanr', margin + 90, y)
      ...
      sorted.forEach(f => {
        ...
        const lev = doc.splitTextToSize(f.leverantor || '', 60)[0] || ''
        doc.text(lev, margin + 26, y)
        doc.text(String(f.fakturanummer || '—'), margin + 90, y)
        ...
        return (
          <div>
            <div style={{ display: 'flex', gap: 8, marginBottom: 12 }}>
              ...
              {levFakturor.length > 0 && <button className="btn-primary" style={{ background: '#1a1a1a', marginTop: 0, flexShrink: 0, width: 'auto', padding: '0 16px' }} onClick={downloadLevPDF} disabled={levPdfLoading}>{levPdfLoading ? '⏳' : '⬇ PDF'}</button>}
            </div>
