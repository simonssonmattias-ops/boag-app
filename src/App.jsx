  ...
  if (view === 'detail' && proj) {
    ...
    return (
      <div>
        ...
        {ptab === 'ekonomi' && isAdmin && <EkonomiPanel projId={proj.id} projName={proj.name} co={co} pTid={pTid} />}
      </div>
    )
  }
...
function EkonomiPanel({ projId, projName, co, pTid }) {
  ...
  const [levFilter, setLevFilter] = useState('alla')
  const [levScanning, setLevScanning] = useState(false)
  const [levPdfLoading, setLevPdfLoading] = useState(false)
  ...
  const downloadLevPDF = async () => {
    setLevPdfLoading(true)
    try {
      const JsPDF = await loadJsPDF()
      const doc = new JsPDF({ orientation: 'portrait', unit: 'mm', format: 'a4' })
      ...
      doc.setTextColor(0, 0, 0)
      doc.setFontSize(16); doc.setFont('helvetica', 'bold')
      doc.text('Leverantörsfakturor', margin, y); y += 7
      doc.setFontSize(11); doc.setFont('helvetica', 'normal'); doc.setTextColor(100, 100, 100)
      doc.text(projName || '', margin, y); y += 8

      // Summering
      const totalt = levFakturor.reduce((s, f) => s + (f.belopp || 0), 0)
      const obetalt = levFakturor.filter(f => f.status !== 'betald').reduce((s, f) => s + (f.belopp || 0), 0)
      const betalt = levFakturor.filter(f => f.status === 'betald').reduce((s, f) => s + (f.belopp || 0), 0)
      const stats = [
        { l: 'Totalt', v: totalt.toLocaleString('sv-SE') + ' kr' },
        { l: 'Obetalt', v: obetalt.toLocaleString('sv-SE') + ' kr' },
        { l: 'Betalt', v: betalt.toLocaleString('sv-SE') + ' kr' },
      ]
      const sw = cw / 3
      stats.forEach((s, i) => {
        const x = margin + i * sw
      ...
      doc.text('Leverantör', margin + 26, y)
      doc.text('Fakturanr', margin + 78, y)
      doc.text('Status', margin + 112, y)
      doc.text('Belopp', margin + cw, y, { align: 'right' })
      y += 3
      doc.setDrawColor(200, 200, 200); doc.line(margin, y, margin + cw, y); y += 5

      // Rader — kronologiskt efter datum (äldst först)
      const sorted = [...levFakturor].sort((a, b) => (a.datum || '').localeCompare(b.datum || ''))
      doc.setFont('helvetica', 'normal'); doc.setTextColor(0, 0, 0)
      sorted.forEach(f => {
        checkPage(12)
        doc.setFontSize(9); doc.setTextColor(0, 0, 0)
        doc.text(fmt(f.datum) || '', margin, y)
        const lev = doc.splitTextToSize(f.leverantor || '', 48)[0] || ''
        doc.text(lev, margin + 26, y)
        doc.text(String(f.fakturanummer || '—'), margin + 78, y)
        if (f.status === 'betald') { doc.setTextColor(26, 122, 60); doc.text('Betald', margin + 112, y) }
        else { doc.setTextColor(163, 45, 45); doc.text('Obetald', margin + 112, y) }
        doc.setTextColor(0, 0, 0); doc.setFont('helvetica', 'bold')
        doc.text((f.belopp || 0).toLocaleString('sv-SE') + ' kr', margin + cw, y, { align: 'right' })
        doc.setFont('helvetica', 'normal')
        y += 5
        if (f.beskrivning) {
          const bl = doc.splitTextToSize(f.beskrivning, cw)
          doc.setFontSize(8); doc.setTextColor(120, 120, 120)
          bl.forEach(line => { checkPage(5); doc.text(line, margin, y); y += 4 })
        }
        y += 2
        doc.setDrawColor(235, 235, 235); doc.line(margin, y, margin + cw, y); y += 4
      })
      ...
      doc.text('Summa', margin, y)
      doc.text(totalt.toLocaleString('sv-SE') + ' kr', margin + cw, y, { align: 'right' })
      ...
      doc.save(`Leverantorsfakturor_${(projName || 'projekt').replace(/\s+/g, '_')}.pdf`)
    } catch (e) { console.error(e); alert('Kunde inte generera PDF') }
    setLevPdfLoading(false)
  }
  ...
  return (
    <div>
      ...
      {section === 'lev' && (() => {
        const levFiltered = levFilter === 'alla' ? levFakturor
          : levFilter === 'betald' ? levFakturor.filter(f => f.status === 'betald')
          : levFakturor.filter(f => f.status !== 'betald')
        const antalObetald = levFakturor.filter(f => f.status !== 'betald').length
        const antalBetald = levFakturor.filter(f => f.status === 'betald').length
        const summaObetald = levFakturor.filter(f => f.status !== 'betald').reduce((s, f) => s + (f.belopp || 0), 0)
        const summaBetald = levFakturor.filter(f => f.status === 'betald').reduce((s, f) => s + (f.belopp || 0), 0)
        const summaVisas = levFiltered.reduce((s, f) => s + (f.belopp || 0), 0)
        return (
          <div>
            <div style={{ display: 'flex', gap: 8, marginBottom: 12 }}>
              <button className="btn-primary" style={{ background: co.color, marginTop: 0, flex: 1 }} onClick={() => setView('levform')}>📷 Lägg till leverantörsfaktura</button>
              {levFakturor.length > 0 && <button className="btn-primary" style={{ background: '#1a1a1a', marginTop: 0, flexShrink: 0 }} onClick={downloadLevPDF} disabled={levPdfLoading}>{levPdfLoading ? '⏳' : '⬇ PDF'}</button>}
            </div>

            <div className="sub-tabs" style={{ marginBottom: 10 }}>
              <button className={`sub-tab${levFilter === 'alla' ? ' active' : ''}`} onClick={() => setLevFilter('alla')}>Alla ({levFakturor.length})</button>
              <button className={`sub-tab${levFilter === 'obetald' ? ' active' : ''}`} onClick={() => setLevFilter('obetald')}>Obetalda {antalObetald > 0 ? `(${antalObetald})` : ''}</button>
              <button className={`sub-tab${levFilter === 'betald' ? ' active' : ''}`} onClick={() => setLevFilter('betald')}>Betalda {antalBetald > 0 ? `(${antalBetald})` : ''}</button>
            </div>

            <div style={{ display: 'flex', gap: 16, fontSize: 12, color: '#888', marginBottom: 12, flexWrap: 'wrap', background: '#F0EFE8', borderRadius: 8, padding: '8px 12px' }}>
              <span>Visas: <strong style={{ color: '#1a1a1a' }}>{summaVisas.toLocaleString('sv-SE')} kr</strong></span>
              <span>Obetalt: <strong style={{ color: '#a32d2d' }}>{summaObetald.toLocaleString('sv-SE')} kr</strong></span>
              <span>Betalt: <strong style={{ color: '#1a7a3c' }}>{summaBetald.toLocaleString('sv-SE')} kr</strong></span>
            </div>

            {levFiltered.length === 0 && <div className="empty">Inga leverantörsfakturor {levFilter !== 'alla' ? 'i denna vy' : 'registrerade ännu'}.</div>}
            {levFiltered.map(f => (
              <div key={f.id} className="card" style={{ marginBottom: 8, opacity: f.status === 'betald' ? 0.65 : 1 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                  <div style={{ flex: 1 }}>
...
function ProjektArbetsdagPanel({ proj, co, dagbok, tid, employees, sess, isAdmin, addDagbok, updateDagbok, addTid, deleteDagbok, deleteTid, deleteTidByEntry, setLb }) {
  ...
  const startEdit = (d) => {
    ...
    const main = linked.find(t => t.employee === d.employee)
    const others = linked.filter(t => t !== main)
    setForm({
          ...
          <div key={d.id} className="entry-card" style={{ marginBottom: 8 }}>
            <div className="card-row">
              <div>
                ...
                {d.text && <div style={{ fontSize: 13, marginTop: 4 }}>{d.text}</div>}
              </div>
