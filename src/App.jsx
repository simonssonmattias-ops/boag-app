    ...
    return (
      <div>
        ...
        <div className="sub-tabs">
          {[{ k: 'info', l: 'Info' }, { k: 'filer', l: `Filer${pFiles.length > 0 ? ` (${pFiles.length})` : ''}` }, { k: 'ata', l: `ÄTA${pendA > 0 ? ' ●' : ''}` }, ...(isAdmin ? [{ k: 'ekonomi', l: 'Ekonomi' }] : [])].map(t => <button key={t.k} className={`sub-tab${ptab === t.k ? ' active' : ''}`} onClick={() => setPtab(t.k)}>{t.l}</button>)}
        </div>
        {ptab === 'info' && isAdmin && (
          <div>
...
function EkonomiPanel({ projId, co, pTid }) {
  ...
  useEffect(() => {
    ...
    ]).then(([f, p]) => { setFakturor(f.data || []); setPlan(p.data || []); setLoaded(true) })
  }, [projId])
  ...
  const totalTimmar = pTid.reduce((s, t) => s + (t.hours || 0), 0)
  ...
  return (
    <div>
      <div className="stat-grid" style={{ marginBottom: 10 }}>
        ...
        <div className="stat-card"><div className="stat-label">Loggade timmar</div><div className="stat-val" style={{ fontSize: 15 }}>{totalTimmar}h</div></div>
      </div>
      <div style={{ display: 'flex', gap: 12, fontSize: 12, color: '#888', marginBottom: 12, flexWrap: 'wrap' }}>
        <span>Betalt: <strong style={{ color: '#1a7a3c' }}>{betalt.toLocaleString('sv-SE')} kr</strong></span>
        <span>Obetalt: <strong style={{ color: '#1a1a1a' }}>{obetalt.toLocaleString('sv-SE')} kr</strong></span>
      ...
      <div style={{ display: 'flex', background: '#F0EFE8', borderRadius: 10, padding: 3, gap: 2, marginBottom: 12 }}>
        {[{ k: 'fakturor', l: `Fakturor (${fakturor.length})` }, { k: 'plan', l: `Betalningsplan (${plan.length})` }].map(s => (
          <button key={s.k} onClick={() => setSection(s.k)} style={{ flex: 1, padding: '8px 4px', border: 'none', borderRadius: 8, fontSize: 12, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit', background: section === s.k ? '#fff' : 'transparent', color: section === s.k ? '#1a1a1a' : '#888' }}>{s.l}</button>
        ))}
      </div>
...
function KorsedlarTab({ co, korsedlar, kunder, employees, sess, isAdmin, addKorsedel, deleteKorsedel, updateKorsedel }) {
  ...
  const totalH = filtered.reduce((s, k) => s + (k.timmar || 0), 0)
