...
export default function App() {
  ...
  return (
    <div className="app" style={{ '--accent': '#1a1a1a' }}>
      ...
      {showSettings && <SettingsSheet employees={employees} addEmployee={addEmployee} deleteEmployee={deleteEmployee} kunder={kunder} addKund={addKund} deleteKund={deleteKund} onClose={() => setShowSettings(false)} />}
...
// ─── Settings Sheet ───────────────────────────────────────────
function SettingsSheet({ employees, addEmployee, deleteEmployee, kunder, addKund, deleteKund, onClose }) {
        ...
        {section === 'personal' && (
          <div>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
              <div style={{ fontSize: 15, fontWeight: 700 }}>{view === 'add' ? 'Lägg till anställd' : 'Personal'}</div>
            ...
            {view === 'add' && (
              <div>
                ...
                <div style={{ display: 'flex', gap: 8 }}>
                  <button className="btn-primary" style={{ flex: 1, marginTop: 0 }} onClick={saveEmp}>Spara</button>
                  <button style={{ marginTop: 0, padding: '10px 16px', background: 'transparent', border: '0.5px solid rgba(0,0,0,.2)', borderRadius: 8, fontSize: 14, cursor: 'pointer', fontFamily: 'inherit' }} onClick={() => { setView('list'); setForm({ name: '', role: 'employee', pin: '', phone: '' }); setPinErr('') }}>Avbryt</button>
                </div>
              </div>
            )}
          </div>
